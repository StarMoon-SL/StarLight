local Library do 
    ...
    do
        ...
        -- Mode Switch (Always/Toggle/Hold)
        Items["ModeSwitch"] = Instances:Create("TextButton", {
            Parent = Items["Keybind"].Instance,
            Name = "\\0",
            FontFace = Library.Font,
            TextColor3 = FromRGB(0, 0, 0),
        ...
        Items["ModeSwitch"]:AddToTheme({BackgroundColor3 = "Element"})
        
        -- Rounded corners for switch
        Instances:Create("UICorner", {
            Parent = Items["ModeSwitch"].Instance,
            Name = "\\0",
            CornerRadius = UDimNew(1, 0)
        })
        ...
        -- Toggle Ball
        Items["SwitchBall"] = Instances:Create("Frame", {
            Parent = Items["ModeSwitch"].Instance,
            Name = "\\0",
            AnchorPoint = Vector2New(0, 0.5),
        ...
        Instances:Create("UICorner", {
            Parent = Items["SwitchBall"].Instance,
            Name = "\\0",
            CornerRadius = UDimNew(1, 0)
        })
        ...
        Items["KeyButton"]:AddToTheme({BackgroundColor3 = "Element", TextColor3 = "Text"})
        ...
        Items["ModeLabel"] = Instances:Create("TextLabel", {
            ...
            BorderSizePixel = 0,
            BackgroundTransparency = 1,
            Position = UDim2New(1, -120, 0, 4),
            BorderColor3 = FromRGB(0, 0, 0),
            ZIndex = 2,
            TextSize = 12,
            BackgroundColor3 = FromRGB(255, 255, 255)
        })
        Items["ModeLabel"]:AddToTheme({TextColor3 = "Text"})
    end
    ...
    function Keybind:SetMode(Mode)
        if table.find(Keybind.Modes, Mode) then
            Keybind.CurrentMode = Mode
            Items["ModeLabel"].Instance.Text = Mode
            
            -- Update switch visual
            if Mode == "Always" then
                Keybind.Toggled = true
                Keybind:UpdateSwitch()
            end
    ...
    function Keybind:UpdateSwitch()
        local IsActive = Keybind.Toggled or Keybind.CurrentMode == "Always"
        local TargetPosition = IsActive and UDim2New(1, -22, 0.5, 0) or UDim2New(0, 2, 0.5, 0)
        local TargetColor = IsActive and Library.Theme.Accent or FromRGB(27, 26, 29)
        
        Items["SwitchBall"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
            Position = TargetPosition
        })
        
        Items["ModeSwitch"]:Tween(TweenInfo.new(Library.Tween.Time, Enum.EasingStyle.Quart, Enum.EasingDirection.Out), {
    ...
    function Keybind:Set(Key)
        if Key == nil then
            ...
            Keybind.Value = Key
            Items["KeyButton"].Instance.Text = Key.Name or tostring(Key)
        end
        Library.Flags[Keybind.Flag] = Keybind.Value
    end
    
    function Keybind:DoClick()
        if Keybind.CurrentMode == "Always" then
            Keybind.Toggled = not Keybind.Toggled
            Keybind:UpdateSwitch()
            Library:SafeCall(Keybind.Callback, Keybind.Toggled, Keybind.Value)
        elseif Keybind.CurrentMode == "Toggle" then
            Keybind.Toggled = not Keybind.Toggled
            Keybind:UpdateSwitch()
            Library:SafeCall(Keybind.Callback, Keybind.Toggled, Keybind.Value)
        elseif Keybind.CurrentMode == "Hold" then
            Keybind.Toggled = true
            Keybind:UpdateSwitch()
            Library:SafeCall(Keybind.Callback, true, Keybind.Value)
        end
    end
    
    function Keybind:DoRelease()
        if Keybind.CurrentMode == "Hold" then
            Keybind.Toggled = false
            Keybind:UpdateSwitch()
            Library:SafeCall(Keybind.Callback, false, Keybind.Value)
        end
    end
    
    -- Mode switch click
    Items["ModeSwitch"]:Connect("InputBegan", function(Input)
        if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
            local CurrentIndex = table.find(Keybind.Modes, Keybind.CurrentMode)
            local NextIndex = CurrentIndex % #Keybind.Modes + 1
            Keybind:SetMode(Keybind.Modes[NextIndex])
        end
    end)
    
    -- Key button click to set key
    ...
    -- Input handling
    Library:Connect(UserInputService.InputBegan, function(Input)
        if Keybind.Value == "None" or Keybind.Value == nil then return end
        
        local IsKey = false
        if typeof(Keybind.Value) == "EnumItem" and Keybind.Value.EnumType == Enum.KeyCode then
            IsKey = Input.KeyCode == Keybind.Value
        elseif Keybind.Value == "MB1" then
            IsKey = Input.UserInputType == Enum.UserInputType.MouseButton1
        elseif Keybind.Value == "MB2" then
            IsKey = Input.UserInputType == Enum.UserInputType.MouseButton2
        end
        
        if IsKey and not UserInputService:GetFocusedTextBox() then
            Keybind:DoClick()
        end
    end)
    
    Library:Connect(UserInputService.InputEnded, function(Input)
        if Keybind.CurrentMode == "Hold" and Keybind.Toggled then
            local IsKey = false
            if typeof(Keybind.Value) == "EnumItem" and Keybind.Value.EnumType == Enum.KeyCode then
                IsKey = Input.KeyCode == Keybind.Value
            elseif Keybind.Value == "MB1" then
